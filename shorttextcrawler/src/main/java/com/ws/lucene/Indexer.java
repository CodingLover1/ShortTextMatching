...
public class Indexer {
     ...
     * @throws Exception
     */
     public  synchronized void indexFile(File files) throws Exception {
    ...
    public  synchronized void indexFile(File file,News news) throws Exception {

        //System.out.println("索引文件："+news.getNewsTitle());

        //索引要一行一行的找，，在数据中为文档，所以要得到所有行，即文档
        Document document = getDocument(file,news);
        //开始写入,就把文档写进了索引文件里去了；
        writer.addDocument(document);
     ...
     * @throws Exception
     * */
    private synchronized Document getDocument(File files) throws Exception {
    ...
    private synchronized Document getDocument(File file,News news) throws Exception {

        //实例化Document
        Document doc = new Document();

        //add():把设置好的索引加到Document里，以便在确定被索引文档。
        doc.add(new TextField("file",new FileReader(file)));
        doc.add(new TextField("newsTitle",news.getNewsTitle(),Field.Store.YES));
        //Field.Store.YES：把文件名存索引文件里，为NO就说明不需要加到索引文件里去
        doc.add(new TextField("newsHref", news.getNewsHref(), Field.Store.YES));

        //把完整路径存在索引文件里
        doc.add(new TextField("newsContent", news.getNewsContent(),Field.Store.YES));

        doc.add(new TextField("newsDate", news.getNewsDate().toString(),Field.Store.YES));
        doc.add(new TextField("className", news.getClassName(),Field.Store.YES));
